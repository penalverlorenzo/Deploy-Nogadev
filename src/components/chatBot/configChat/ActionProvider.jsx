...
const ActionProvider = ({ createChatBotMessage , setState, children }) => {
  const { sessionId } = useContext(DataProvider);
  const handleHello = () => {
  ...
  const generatedPrompt = async (message) => {
    try {
      const res = await GeneratedPromptAnswer(message, language, sessionId);
      let res2 = res;
      const response = res2 ? res2 : res2.response
      const botMessage = createCustomMessage('Holaaa', 'customMessage', { payload: { message: response } })
      setState((prev) => ({
        ...prev,
        messages: [...prev.messages, botMessage],
      }));
